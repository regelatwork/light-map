...
class CameraOperator:
    ...
    def __init__(self, width: int = 1920, height: int = 1080, num_consumers: int = 2):
        ...
        logging.info(f"CameraOperator initialized with shared memory: {self.shm_name}, size={self.total_size}")
    ...
    def _publish_frame(self, frame: np.ndarray, timestamp: int) -> int:
        ...
        shm_frame = np.frombuffer(shm_frame_buf, dtype=np.uint8).reshape((self.height, self.width, 3))
