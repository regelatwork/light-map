...
2.  **Integer Blending Formula**: Use a bit-shift approximation for alpha blending:
    `Result = ((Patch * Alpha) + (ROI * (256 - Alpha))) >> 8`
3.  **SIMD Optimization**: Leverage NumPy's vectorized operations to perform these integer calculations across entire arrays.
