...
1. **Menu Trigger:** Open the `TRIGGER_MENU` (Action). This provides a dark background which significantly improves ArUco contrast for the camera.
1. **Stabilization Time:** Keep the menu open for at least 5-10 seconds.
1. **Coordinate Query:** Use `GET /state/tokens` to find the actual `world_x, world_y` of the physical tokens.
1. **Dynamic Pointing:** Point at the *detected* coordinates, not hardcoded ones.
