...
def test_extrinsics_scene_validation_flow(
    mock_save, mock_calibrate, mock_context
):
    # Setup
    ...
    # Mock return: rvec, tvec, obj_points, img_points
    obj_pts = np.array([[0,0,0], [10,0,0], [0,10,0], [0,0,10]], dtype=np.float32)
    img_pts = np.array([[100,100], [110,100], [100,110], [100,100]], dtype=np.float32) # Dummy
    rvec = np.zeros((3,1))
    tvec = np.zeros((3,1))
