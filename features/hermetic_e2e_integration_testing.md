...
1. Seeding initial configuration (if provided via `--config-dir`).
1. Starting the backend via `xvfb-run` (to support headless environments).
1. Injecting initial test state (e.g., tokens) via the REST API.
1. Executing Playwright tests against the live servers.
1. Managing the lifecycle (startup/shutdown/cleanup) of all processes.
...
1. Inspect `backend_e2e.log` in that directory to find Python tracebacks.
1. Check the `config/` and `data/` subdirectories to verify that the backend's internal state matches expectations.
