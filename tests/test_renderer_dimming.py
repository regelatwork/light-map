...
def test_render_opacity_zero(mock_menu_state):
    ...
    out = renderer.render(mock_menu_state, background=bg, map_opacity=0.0)
...
def test_render_opacity_full(mock_menu_state):
    ...
    out = renderer.render(mock_menu_state, background=bg, map_opacity=1.0)
...
def test_render_opacity_half(mock_menu_state):
    ...
    out = renderer.render(mock_menu_state, background=bg, map_opacity=0.5)
    mean = np.mean(out)
...
def test_render_no_background_ignores_opacity(mock_menu_state):
    ...
    out = renderer.render(mock_menu_state, background=None, map_opacity=0.5)
    assert np.mean(out) == 0
