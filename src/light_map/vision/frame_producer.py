...
class FrameProducer:
    ...
    """
    def __init__(self, shm_name: str, width: int = 1920, height: int = 1080, num_consumers: int = 2):
        self.width = width
        self.height = height
        ...
        self._ref_counts = np.frombuffer(self.shm.buf[self.ctrl_ref_offset : self.ctrl_ts_offset], dtype=np.int32)
        self._timestamps = np.frombuffer(self.shm.buf[self.ctrl_ts_offset : self.ctrl_latest_offset], dtype=np.int64)
        self._latest_id = np.frombuffer(self.shm.buf[self.ctrl_latest_offset : self.control_block_size], dtype=np.int32)
    ...
    def get_latest_frame(self) -> Optional[np.ndarray]:
        ...
        self._current_frame_view = np.frombuffer(shm_frame_buf, dtype=np.uint8).reshape((self.height, self.width, 3))
    ...
    def close(self):
        ...
        if hasattr(self, '_ref_counts'): del self._ref_counts
        if hasattr(self, '_timestamps'): del self._timestamps
        if hasattr(self, '_latest_id'): del self._latest_id
