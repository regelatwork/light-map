...
1. **Extract MockWebSocket:** Create `frontend/e2e/utils/mock-socket.ts`.
...
1. **Define Shared E2E Types:** Create `frontend/e2e/types/e2e.ts` for interfaces like `E2EWindow` (extending `Window` to include `VITE_API_HOST`).
...
1. **Execution:** `npx playwright test`.
