...
import cv2
import pytest
from light_map.vision.projection import CameraProjectionModel, Projector3DModel, ProjectionService
...
def test_projection_service_parallax_logic():
    ...
    proj_rvec = rvec.copy()
    proj_tvec = np.array([[200], [0], [1000]], dtype=np.float32) # t is in camera coords? 
    ...
    proj_pixels = service.project_camera_to_projector(cam_pixels, height_mm=0, target_z=0, prefer_homography=False)
    ...
    proj_pixels_h100 = service.project_camera_to_projector(cam_pixels, height_mm=100, target_z=100, prefer_homography=False)
    ...
    proj_pixels_shadow = service.project_camera_to_projector(cam_pixels_offset, height_mm=100, target_z=0, prefer_homography=False)
...
def test_projection_service_homography_parallax():
    ...
    proj_pixels = service.project_camera_to_projector(cam_pixels, height_mm=100, target_z=100, prefer_homography=True)
    
    assert proj_pixels[0, 0] == pytest.approx(277.777, abs=1.0) # Allow some tolerance for homography vs 3D
