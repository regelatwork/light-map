...
def run_all():
    ...
    for filename in sorted(os.listdir(cases_dir)):
        ...
        act_coverage = (np.count_nonzero(cv2.max(act_img[:,:,0], cv2.max(act_img[:,:,1], act_img[:,:,2]))) / (act_img.size/3)) * 100
        ref_coverage = (np.count_nonzero(cv2.max(ref_img[:,:,0], cv2.max(ref_img[:,:,1], ref_img[:,:,2]))) / (ref_img.size/3)) * 100
