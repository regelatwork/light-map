...
def test_tall_object_first_exit_logic():
    ...
    mask_visible, _ = engine.get_token_vision_mask(
        ...
        vision_range_grid=50.0, mask_width=400, mask_height=400
    )
    ...
    assert mask_visible[target_my2, target_mx2] == 0, "Area behind plateau should be blocked from ground"
    ...
    mask_visible_from_top, _ = engine.get_token_vision_mask(
        token_id=2, origin_x=12.0, origin_y=12.0, size=1, 
        vision_range_grid=50.0, mask_width=400, mask_height=400
    )
    
    ground_mx = int(5.0 * engine.svg_to_mask_scale)
    ground_my = int(5.0 * engine.svg_to_mask_scale)
    assert mask_visible_from_top[ground_my, ground_mx] == 255, "Ground should be visible from plateau top"
...
def test_tall_object_multiple_exit_blocking():
    ...
    mask, _ = engine.get_token_vision_mask(
        token_id=3, origin_x=15.0, origin_y=10.0, size=1,
        vision_range_grid=100.0, mask_width=800, mask_height=400
    )
    ...
    assert mask[my1, mx1] == 255, "Gap between tall objects should be visible from first tall object"
