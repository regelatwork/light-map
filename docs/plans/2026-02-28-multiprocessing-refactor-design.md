...
- **Update Logic**: The `update_from_frame(shm_view, timestamp)` method:
  ...
  1. Updates its internal background buffer with the result.
  1. Sets a granular `dirty_background` flag to `True`.
  1. Records the `timestamp` to ensure synchronization with vision results.
...
1. **State Application**: Calls `world_state.apply(result)` for each item.
1. **Synchronization**: `WorldState` uses the `timestamp` to ensure it only applies results that are newer than its current state.
1. **Dirty Flagging**: As `WorldState` updates its data, it sets granular flags (e.g., `dirty_tokens = True`).
...
1. **SHM** -> `Detectors` (SHM Read via `FrameProducer`).
1. **Detectors** -> `ResultQueues` (IPC).
1. **MainLoop** -> `WorldState` / `InputManager`.
1. **WorldState** -> `Renderer` (via `dirty` flags).
