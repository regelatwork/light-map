...
def visualize_distortion(calib_file="projector_calibration.npz", output_file="distortion_field.png"):
    ...
    print(f"Distortion Statistics (Residuals):")
    ...
    for i in range(len(theoretical_proj_pts)):
        ...
        cv2.arrowedLine(img, (int(round(p1[0])), int(round(p1[1]))), 
                        (int(round(p2[0])), int(round(p2[1]))), color, 2, tipLength=0.3)

    cv2.putText(img, f"Max Residual: {np.max(magnitudes):.2f}px (Vector Scale: {scale}x)", 
