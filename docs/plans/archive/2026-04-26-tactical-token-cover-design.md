...
1. **Token Stamping:** We will stamp the footprints of all active tokens (excluding the attacker and target) onto this temporary mask using a new constant `MASK_VALUE_SOFT_COVER = 75`.
1. **Trace Update:** The Numba ray-tracing function will be updated to recognize this new value and return a distinct "Soft Cover" status, allowing the engine to calculate AC and Reflex bonuses independently.
...
1. **Scene Integration (`exclusive_vision_scene.py`)**
