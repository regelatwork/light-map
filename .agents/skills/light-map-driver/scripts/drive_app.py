...
def main():
    ...
    parser.add_argument("--tokens", type=int, nargs="+", default=[4, 11, 12], help="Token IDs to test")
    ...
    # 1. Start App
    cmd = ["python3", "-m", "light_map", "--debug", "--remote-hands", "exclusive", 
           "--remote-tokens", "merge", "--map", args.map, "--log-level", "DEBUG"]
    ...
    try:
        ...
        for tid in args.tokens:
            if tid in detected:
                ...
                for _ in range(int(args.dwell * 2)):
                    httpx.post(f"{args.url}/input/hands/world", json=[{
                        "world_x": wx, "world_y": wy, "gesture": "Pointing",
        ...
        for log in logs:
            if "[ExclusiveVision]" in log: print(log)

    finally:
        print("Shutting down...")
        try: httpx.post(f"{args.url}/input/actions", json=["QUIT"], timeout=2)
        except: pass
