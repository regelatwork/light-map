...
\begin{enumerate}
    ...
    \item 	extbf{Extrinsics Calibration}: This step defines the spatial relationship between the camera and the projection surface, enabling the transformation of pixel coordinates into physical world coordinates.
    ...
    \item 	extbf{Scale Calibration}: A critical final step is determining the Pixels Per Inch (PPI) scale to ensure metric consistency, allowing digital grids and tokens to match physical dimensions accurately.
\end{enumerate}
...
For unmarked, physical objects, we utilize a structured light approach. The projector emits a jittered staggered (hexagonal) dot grid pattern, and the camera captures the surface. By comparing the pattern against a dark frame baseline, applying dynamic thresholding, and removing expected dots from the background, the system isolates physical objects that occlude or distort the projection. These observed centroids are mapped through the projector transformation with global drift and missing dot correction to cluster and identify tokens.
