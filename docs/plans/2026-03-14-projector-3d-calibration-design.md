...
1. **Multi-Marker Projection**: Projects a $3 \\times 2$ ArUco grid on the box ($Z=H$) and reference markers on the table ($Z=0$).
1. **Gesture-Triggered Capture**: User performs the **Victory** gesture to sample points.
1. **3D Reconstruction**: Camera rays are intersected with $Z=H$ or $Z=0$ planes to find World Coordinates $(X, Y, Z)\_w$.
1. **Solver**: Uses `cv2.calibrateCamera` to calculate $K_p, D_p, R_p, t_p$.
