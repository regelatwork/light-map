...
def generate_labeled_overlay():
    ...
    map_system.set_state(
        viewport["x"], viewport["y"], viewport["zoom"], viewport["rotation"]
    )
    ...
    for marker_id in unique_markers:
        ...
        if M["m00"] != 0:
            ...
            # Draw Outline
            color = (
                (0, 255, 0) if is_snapped else (0, 165, 255)
            )  # Green if snapped, Orange if not
            cv2.drawContours(vis, contours, -1, color, 2)

            # Draw Label
            cv2.putText(
                vis, label, (cx - 10, cy), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2
            )
            cv2.putText(
                vis,
                sub_label,
