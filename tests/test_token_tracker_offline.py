...
def test_token_tracker_offline_detection():
    ...
    for t in tokens:
        print(f"  Token {t.id}: World({t.world_x:.1f}, {t.world_y:.1f}) -> Grid({t.grid_x}, {t.grid_y})")
    ...
    for exp in expected:
        ...
        for t in tokens:
            if exp["grid_x"] is not None:
                ...
                # Match tokens with no grid snap
                if t.grid_x is None and t.grid_y is None:
                    # To avoid double counting, check if this token was already matched?
                    # Since we are iterating *expectations*, one expectation matches one token.
                    # Ideally we should remove tokens from 'tokens' list as we match them.
                    pass 
        
        # Simple list filtering to properly count matches
        if exp["grid_x"] is not None:
             candidates = [t for t in tokens if t.grid_x == exp["grid_x"] and t.grid_y == exp["grid_y"]]
             if candidates:
                 found_count += 1
             ...
             if candidates:
                 found_count += 1
                 tokens.remove(candidates[0])
                 print(f"  [OK] Found Unsnapped Token")
             else:
                 print(f"  [FAIL] Missing Unsnapped Token")
