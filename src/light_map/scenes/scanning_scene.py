...
from light_map.common_types import SceneId, SessionData, ViewportState, TokenDetectionAlgorithm
...
class ScanningScene(Scene):
    ...
    def update(
        ...
        # --- Structured Light Sequence ---
            ...
            if self._pattern_image is None:
                ...
                random.seed(42) 
                self._pattern_image, self._cached_pattern_points = self.token_tracker.get_scan_pattern(w, h, ppi)
    ...
    def render(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self._stage in [ScanStage.PREPARE_DARK, ScanStage.WAIT_DARK, ScanStage.CAPTURE_DARK]:
             return np.zeros_like(frame, dtype=np.uint8)

        if self._stage in [ScanStage.PREPARE_PATTERN, ScanStage.WAIT_PATTERN, ScanStage.CAPTURE_PATTERN]:
