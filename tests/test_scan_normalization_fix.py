...
def test_interactive_app_resolution_sync():
    m = np.eye(3, dtype=np.float32)
    config = AppConfig(width=640, height=480, projector_matrix=m)
...
def test_interactive_app_load_map_normalization(tmp_path):
    ...
    # Setup
    m = np.eye(3, dtype=np.float32)
    config = AppConfig(width=100, height=100, projector_matrix=m)
    app = InteractiveApp(config)
