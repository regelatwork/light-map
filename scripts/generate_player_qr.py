...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a US-Letter sized QR code for the Player Tactical Dashboard."
    )
    parser.add_argument(
        "--host", type=str, default="127.0.0.1", help="Host address of the Light Map server"
    )
    parser.add_argument("--port", type=int, default=8000, help="Port of the Light Map server")
    parser.add_argument(
        "--output",
        type=str,
        default="player_qr.svg",
        help="Output file path (default: player_qr.svg)",
    )
...
"""
    ...
    if args.pdf:
        ...
        print("Exporting to PDF using Inkscape...")
        try:
            subprocess.run(
                ["inkscape", args.output, "--export-filename=" + pdf_path],
                check=True,
                capture_output=True,
            )
