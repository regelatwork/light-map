...
class TokenTracker:
    ...
    def _detect_structured_light(
        ...
        if self.debug_mode:
             ...
             print(f"Debug: Projector Matrix: [{m_flat[0]:.2f}, {m_flat[1]:.2f}, {m_flat[2]:.2f}]")
             print(f"                        [{m_flat[3]:.2f}, {m_flat[4]:.2f}, {m_flat[5]:.2f}]")
             print(f"                        [{m_flat[6]:.2f}, {m_flat[7]:.2f}, {m_flat[8]:.2f}]")
        ...
        if self.debug_mode:
            print(f"Debug: Max Diff Brightness (ROI): {max_val}. Threshold used: {dynamic_thresh if max_val >=30 else '0'}") 
        ...
        if len(dst_pts) > 0:
             ...
             if self.debug_mode:
                 print(f"Debug: Observed Points Range X:[{min(xs):.1f}, {max(xs):.1f}] Y:[{min(ys):.1f}, {max(ys):.1f}]")
        ...
        max_shift_observed = np.max(all_shift_magnitudes) if all_shift_magnitudes else 0.0
        print(f"Debug: Max Shift Candidate: {max_shift_observed:.2f}px. (Threshold: {SHIFT_THRESHOLD}px)")
        ...
        if detected_tokens_points:
             ...
             for cluster in clusters:
                 ...
                 tokens.append(Token(
                     ...
                     world_x=wx,
                     world_y=wy,
                     confidence=min(1.0, len(cluster) / 3.0) # Confidence increases with points
                 ))
                 token_id += 1
    ...
    def _save_debug_image(
        ...
        grid_origin_x: float,
        grid_origin_y: float,
        debug_vectors: Optional[List[Tuple[Tuple[float, float], Tuple[float, float]]]] = None,
        ...
        if debug_vectors:
            ...
            if displacements:
                ...
                print(f"Displacement: Min={d_arr.min():.2f}, Max={d_arr.max():.2f}, Mean={d_arr.mean():.2f}, Median={np.median(d_arr):.2f}")
                ...
                print(f"Radial Analysis (Barrel Distortion Check):")
                ...
                print(f"  Max Error Location: ({max_pt[0]:.0f}, {max_pt[1]:.0f}) -> {d_arr[max_idx]:.2f}px")
        ...
        if expected_points:
            for ep in expected_points:
                cv2.circle(debug_img, (int(ep[0]), int(ep[1])), 3, (255, 255, 0), -1) # Cyan/Yellow dots (thicker 3px)
