...
1. **Camera Capture Process**: Dedicated to high-speed frame retrieval via GStreamer/OpenCV.
1. **Vision Worker Processes**: Independent processes for Hand Landmarks and ArUco Token detection.
...
1. **SHM** -> `Workers` (SHM Read via `FrameProducer`).
1. **Workers** -> `ResultQueues` (IPC).
1. **MainLoop** -> `WorldState` (Apply Results & ROI Extraction).
1. **MainLoop** -> `Renderer` (Render to Projector).
