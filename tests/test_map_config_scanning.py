...
import os
from light_map.map_config import MapConfigManager, MapEntry
...
def test_scan_for_maps_prunes_missing_maps(mock_config_file):
    ...
    with (
        ...
        def exists_side_effect(path):
            if path == "/maps/new.svg": return True
            if path == "/maps/old.svg": return False
            if path == "test_map_state.json": return False # For initial load check
            return False
