...
1. **Camera Exposure**: A sudden bright flash causes auto-exposure to darken the image, potentially losing token details.
...
1. **UI Blocking**: The flash is disruptive. The UI must block interaction during the scan.
...
- **State Machine**:
  ...
  1. **Enter SCANNING Mode**: Stop rendering map/menu.
  1. **Render White**: Output full white frame.
  1. **Wait**: Sleep 0.5s (or X frames).
  1. **Capture**: Read frame from `Camera`.
  1. **Process**: Call `TokenTracker.detect_tokens()`.
  1. **Restore**: Switch back to `AppMode.MENU` / `AppMode.MAP`.
...
1. **Blur**: Gaussian Blur (9x9) to reduce internal texture noise.
...
1. **Morphology**: Open then Close to remove noise and fill holes.
1. **Distance Transform**: Centers of tokens become bright peaks.
1. **Watershed**: Use peaks as markers to segment touching blobs.
...
1. **Division**: Split the bounding box geometrically and assign centers to sub-regions.
...
1. **Feedback**: "Stand Clear... Scanning..." -> White Flash -> "Saved X Tokens".
