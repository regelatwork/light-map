...
To ensure stable tracking, we apply temporal filtering, occlusion buffering, and confidence scoring. For structured light detection, token candidates are clustered using continuous rectangle placement and mean-shift clustering. When grid alignment is enabled, all tracked tokens snap to the virtual grid, preventing duplicate assignments to a single cell and ensuring smooth overlays.
