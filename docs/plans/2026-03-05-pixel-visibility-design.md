...
1. **Corner Peeking:** Tokens pushed against a corner can illuminate the adjacent hallway.
1. **No Spillover:** Light does not "spill" through a wall even if the wall is not perfectly aligned with the grid.
1. **Performance:** Vision calculation remains efficient for "Sync Vision" and "Token Inspection" workflows (targeting \<100ms per token).
