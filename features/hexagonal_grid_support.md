...
- **Axial-to-Pixel Transformation**:
  ...
  - *Note*: `size` is the radius to a vertex ($vertex_dist = spacing / \\sqrt{3}$).
...
- **Hex Rounding (Cube Rounding)**:
  ...
  1. Round $x, y, z$ to the nearest integers $rx, ry, rz$.
  1. Calculate differences $dx, dy, dz$.
  1. Adjust the coordinate with the largest difference to satisfy $rx+ry+rz=0$.
  1. Convert back to axial.
