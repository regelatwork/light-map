...
from src.light_map.menu_system import MenuSystem, MenuSystemState
from src.light_map.common_types import MenuItem, MenuActions, GestureType
from src.light_map.menu_config import (
    LOCK_DELAY, SUMMON_TIME, PRIMING_TIME, SELECT_GESTURE, SUMMON_GESTURE
)
    ...
    return MenuItem(
        ...
        children=[
            ...
            MenuItem("Submenu", children=[
                MenuItem("SubItem 1", action_id="SUB_ACTION_1")
            ])
        ]
    )
...
def test_navigation_and_hover(menu_system, mock_time):
    ...
    # Hover Item 1
    state = menu_system.update(500, 400, GestureType.OPEN_PALM) # Y=400 is inside Item 1
    assert state.hovered_item_index == 0
    assert state.active_items[0].title == "Item 1"
    
    # Hover Item 2
    state = menu_system.update(500, 500, GestureType.OPEN_PALM) # Y=500 is inside Item 2
