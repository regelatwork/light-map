...
from light_map.core.calibration.utils import get_marker_partition, compute_roi_pass2
...
class SequentialSolver:
    ...
    def solve(self, 
               ...
               tokens_json: Dict) -> CalibrationResult:
        """
        Executes the sequential solver.
        """
        # Phase 1: Table Scale & Z=0 Homography
        projector_ppi, table_points_list, table_homography = self._solve_phase_1(projected_grid_detections, ppi_ruler_detections, projected_grid_detections)
        ...
        extrinsics = self._solve_phase_2(token_detections, projected_grid_detections, table_points_list, tokens_json)
        ...
        angle = np.arccos(np.clip((R_stereo_raw[0, 0] + R_stereo_raw[1, 1] + R_stereo_raw[2, 2] - 3) / 3, -1.0, 1.0))
        if angle > np.radians(10):
            logger.warning("Large rotation angle detected between cameras: %f degrees", np.degrees(angle))
        ...
        # ROI Calculation
        roi_left, roi_right = self._calculate_rois(pts_3d, pts_2d_l, pts_2d_r, R_stereo, T_stereo, projector_ppi)

        return CalibrationResult(
            camera_left_intrinsics=self.K_L,
            camera_right_intrinsics=self.K_R,
    ...
    def _solve_phase_1(self, grid_dets: List[Dict], ruler_dets: List[Dict], projected_grid_detections: List[Dict]) -> Tuple[float, List[np.ndarray], np.ndarray]:
        """
        ...
        if mid == 40:
            if side == "left":
                ...
                p40_r = np.array(det['points'], dtype=np.float32)
        elif mid == 41:
            if side == "left":
                p41_l = np.array(det['points'], dtype=np.float32)
            else:
                p41_r = np.array(det['points'], dtype=np.float32)
        ...
        homography, _ = cv2.findHomography(np.array(table_points[:, :2], dtype=np.float32), np.array(grid_pixel_points, dtype=np.float32))
    
        return ppi, table_points.tolist(), homography
    
    def _solve_phase_2(self, token_dets: List[Dict], grid_dets: List[Dict], table_pts_list: List[np.ndarray], tokens_json: Dict) -> Dict[str, np.ndarray]:
        """
        ...
        pts_l = np.vstack([pts_2d_l[mid] for mid in common_ids]).astype(np.float32)
        pts_r = np.vstack([pts_2d_r[mid] for mid in common_ids]).astype(np.float32)
        ...
        logger.error("Final shapes - pts_l: %s, pts_r: %s, pts_3d_common: %s", pts_l.shape, pts_r.shape, pts_3d_common.shape)
        ...
        r_stereo, t_stereo, _ = cv2.stereoCalibrate(
            pts_3d_common, pts_l, pts_r, self.K_L, self.dist_L, self.K_R, self.dist_R,
            (1920, 1080), flags=cv2.CALIB_FIX_INTRINSIC
        )
        
        return {'R': r_stereo, 't': t_stereo}

    def _calculate_rois(self, pts_3d: Dict[int, np.ndarray], pts_2d_l: Dict[int, np.ndarray], 
                         pts_2d_r: Dict[int, np.ndarray], r_stereo: np.ndarray, 
                         t_stereo: np.ndarray, projector_ppi: float) -> Tuple[tuple, tuple]:
        """
        ...
        # Project to Camera L
        img_pts_l, _ = cv2.projectPoints(all_3d_pts, r_world_to_l, t_world_to_l, self.K_L, self.dist_L)
        ...
        img_pts_r, _ = cv2.projectPoints(all_3d_pts, r_world_to_r, t_world_to_r, self.K_R, self.dist_R)
