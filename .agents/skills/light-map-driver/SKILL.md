...
## name: light-map-driver description: Automate and drive the Light Map application via its Remote Driver API. Use when debugging tactical logic, verifying token detection, or performing end-to-end integration tests that require starting the app, stabilizing ArUco detection, and simulating hand/token inputs.
...
1. **Interact:** Inject tokens (mock) or Query tokens (physical) and simulate a "Dwell" by pointing at coordinates for 2+ seconds.
1. **Verify:** Check logs and state for tactical results.
1. **Shutdown:** Gracefully close the app using the `QUIT` action.
