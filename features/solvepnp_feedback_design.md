...
- **Formula**: $RMS = \\sqrt{\\frac{1}{N} \\sum\_{i=1}^{N} ||p\_{detected}^{(i)} - p\_{reprojected}^{(i)}||^2}$
...
1. **Reprojected Points**: Marked with a **Red Circle** (○).
1. **Residual Vectors**: A line connecting the Green Cross to the Red Circle.
