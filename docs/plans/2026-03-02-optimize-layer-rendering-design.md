...
- **Composition Loop:**
  ...
  1. If static layers are dirty, update `background_cache`.
  1. Copy `background_cache` to `output_buffer`.
  1. Blend dynamic layers (e.g., `HandMaskLayer`, `MenuLayer`) on top of `output_buffer`.
...
1. **Dirty Check:** `Renderer.render()` polls all layers in the stack for their `is_dirty` status.
1. **Static Flattening:** If any static layer is dirty, the `Renderer` clears and re-composites all static layers into the `background_cache`.
1. **Dynamic Blending:** `Renderer` copies `background_cache` to `output_buffer` and blends dynamic layer patches.
1. **Idle State:** If no changes occur, the `InteractiveApp` receives `None` and skips the hardware display update.
