...
def test_calibrate_extrinsics_synthetic():
    ...
    proj_coords = [[100, 100], [500, 100], [100, 400], [500, 400]]
    ...
    img_points, _ = cv2.projectPoints(
        obj_points, rvec_true, tvec_true, camera_matrix, dist_coeffs
    )
    ...
    with pytest.MonkeyPatch.context() as mp:
        ...
        ground_points[:, 2] = 0
        img_points_ground, _ = cv2.projectPoints(
            ground_points, rvec_true, tvec_true, camera_matrix, dist_coeffs
        )
        img_points_ground = img_points_ground.reshape(-1, 2)

        projector_matrix, _ = cv2.findHomography(
            img_points_ground, np.array(proj_coords, dtype=np.float32)
        )
