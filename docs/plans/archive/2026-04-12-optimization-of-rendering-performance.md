...
1. **Stable Layer Lifecycle:** Remove re-instantiation logic in `LayerStackManager`. Layers are created once, ensuring stable object identities for the `Renderer` and `CompositeLayer`.
1. **New Layer Mode:** Add `LayerMode.MASKED` for binary masks (0 or 255) to enable a high-performance composition fast-path.
1. **Shared Composition Utility:** Extract the composition logic (including `MASKED` indexing and `NORMAL` alpha blending) to a shared utility used by both `Renderer` and `CompositeLayer`.
1. **Versioning Refinement:** Stabilize `HandMaskLayer` and `ArucoMaskLayer` versioning to prevent unnecessary foreground re-composition.
