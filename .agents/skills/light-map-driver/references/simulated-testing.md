...
1. **API Logs:** Fetch `GET /state/logs` and grep for `[ExclusiveVision]` or `[TacticalOverlay]`.
1. **State Check:** Query `GET /state/tactical_bonuses` to verify the calculated (AC, Reflex) tuples.
