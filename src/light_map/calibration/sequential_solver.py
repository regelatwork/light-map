...
class SequentialSolver:
    def __init__(self, token_manager: TokenManager, projector_ppi: float, projector_matrix: np.ndarray, pattern_params: dict):
    ...
    def solve_phase1_table_scale(self, 
        ...
        grid_map = {id: corners for id, corners in all_markers if id in [42, 43, 44, 45, 46, 47, 48, 49]}
        
        # Check for horizontal pairs (e.g., 42-43, 44-45, 46-47)
        for i in range(3):
            id1, id2 = 42 + 2*i, 43 + 2*i
            if id1 in grid_map and id2 in grid_map:
                dist_px = np.linalg.norm(np.mean(grid_map[id1], axis=0) - np.mean(grid_map[id2], axis=0))
    ...
    def solve_phase2_stereo_extrinsics(self, 
                                          left_marker_corners: List[np.ndarray], 
                                          right_marker_corners: List[np.ndarray],
                                          token_heights: Dict[str, float]) -> None:
        """
        ...
        if len(left_marker_corners) != 12 or len(right_marker_corners) != 12:
            raise ValueError(f"Expected 12 markers (8 grid + 4 tokens), but found {len(left_marker_corners)} left and {len(right_marker_corners)} right.")
        
        pts_l = np.array([m for m in left_marker_corners], dtype=np.float32)
        pts_r = np.array([m for m in right_marker_corners], dtype=np.float32)
        ...
        self.camera_right_t = (self.r_stereo @ self.camera_left_t.reshape(3, 1)).flatten() + self.t_stereo
