...
2.  **Dynamic Updates**: Automatically update the list when files are added via CLI or removed from disk.
3.  **Status Indicators**: Visually denote which maps have:
    *   **Scale/Grid Calibration**: Grid size and origin are set.
    *   **Saved Sessions**: Active token placements saved.
4.  **Session Persistence**: Save and load token configurations specific to each map.
5.  **Streamlined UX**: Simplify the process of "Starting a New Game" vs "Continuing a Session".
...
2.  **Load Session** (Visible if `has_session` is true):
...
2.  **System**: `MapConfigManager` expands glob, updates `map_state.json`, pruning missing maps.
3.  **User**: Selects "Maps" -> "Dungeon Level 1 (!)".
4.  **System**: Shows Action Menu for "Dungeon Level 1".
5.  **User**: Selects "Calibrate Scale".
6.  **System**: `InteractiveApp` parses `CALIBRATE_MAP|...`, loads map, enters Calibration Mode. User sets 1-inch grid.
7.  **System**: Saves grid config. Updates Map List status (removes `(!)`).
8.  **User**: Places tokens, plays.
9.  **User**: Selects "Session" -> "Save".
10. **System**: Saves token positions to `sessions/dungeon_level_1_....json`. Updates Map List status (adds `(*)`).
